...
def calculate_quality_metrics(predictions: Iterable[PredictionResult]) -> QualityReport:
    ...
    precision = true_ng / predicted_ng if predicted_ng else None
    ...
    metrics: dict[str, float | int | str | None] = {
        "NG Tested": len(actual_ng),
        "NG Detected": true_ng,
        "NG Missed": escaped_ng,
        "NG Detection Rate": recall,
        "Escape Rate": escaped_ng / len(actual_ng) if actual_ng else None,
        ...
        "Actual NG -> Predicted OK (Escaped NG)": escaped_ng,
        "Actual NG -> Predicted NG": true_ng,
        "Precision": precision,
        "Recall": recall,
        "F1": f1,
        "AUROC": _auroc(items),
    ...
    if escaped_ng:
        ...
        status = "WARNING"
    return QualityReport(metrics=metrics, status=status)
